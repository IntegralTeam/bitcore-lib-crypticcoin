...
BlockHeader._fromObject = function _fromObject(data) {
  ...
  if (_.isString(data.round)) {
    round = BufferUtil.reverse(new Buffer(data.round, 'hex'));
  }
...
BlockHeader._fromBufferReader = function _fromBufferReader(br) {
  ...
  if (info.version >= SAPLING_BLOCK_VERSION) {
      info.round = br.readUInt32LE();
  }
...
BlockHeader.prototype.toBufferWriter = function toBufferWriter(bw) {
  ...
  if (this.version >= SAPLING_BLOCK_VERSION) {
    bw.writeUInt32LE(this.round);
  }
  bw.write(this.nonce);
