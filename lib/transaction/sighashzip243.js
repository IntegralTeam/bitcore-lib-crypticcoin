...
var PREVOUTS_HASH_PERSON = Buffer.from('CryptPrevoutHash')
var SEQUENCE_HASH_PERSON = Buffer.from('CryptSequencHash')
var OUTPUTS_HASH_PERSON = Buffer.from('CryptOutputsHash')
// var JOINSPLITS_HASH_PERSON = Buffer.from('CryptJSplitsHash')
// var SHIELDEDSPENDS_HASH_PERSON = Buffer.from('CryptSSpendsHash')
// var SHIELDEDOUTPUTS_HASH_PERSON = Buffer.from('CryptSOutputHash')
// var OVERWINTER_HASH_PERSON = Buffer.concat([Buffer.from('CryptSigHash'), Buffer.from('191ba85b', 'hex')])
var SAPLING_HASH_PERSON = Buffer.concat([Buffer.from('CryptSigHash'), Buffer.from('bb09b876', 'hex')])
...
var sighash = function sighash(transaction, sighashType, inputNumber, scriptCode, satoshisBuffer) {
  ...
  writer.writeUInt32LE(transaction.version + (transaction.fInstant ? 0xC0000000 : 0x80000000));
